function st_boxdotplot(x,y,dotCol,ciMeth,nBoot,bxCol,bxFA,bxWth,dotSz,jitSz,lnWth, Fill)
        %st_boxdotplot([1:4],RTmat,COLORS,'iqr',[],[],[],[],6,[],[])
...
%   Fill - filled dots (or empty circles)
...
for i = 1:length(y)
    ...
    if Fill == 1
        ...
        scatter(x(i)+jitter,y{i},dotSz,dotCol(i,:))
    end
  hold on;
   % scatter(x(i)+jitter,y{i},dotSz,dotCol(i,:),'fill')
